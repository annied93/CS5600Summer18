...
static int mem_check(unsigned char *ptr, size_t size)
{
	...
	if (ptr[size-1] != ((j ^ (j>>8)) & 0xFF)) {
		printf("ptr=%p size=%zu, j=%zu j>>8=%d, ptr[size-1]=%d\n", size,j, (j>>8), ptr[size-1]);
		 return 2;
	}
	return 0;
}
...
static void bin_test(struct bin_info *p)
{
	...
	for (b = 0; b < p->bins; b++)
	{
		int ret = mem_check(p->m[b].ptr, p->m[b].size);
		if (mem_check(p->m[b].ptr, p->m[b].size))
		{
			printf("memory corrupt! 2 ret=%d\n",ret);
            //malloc_stats();
			exit(1);
		}
	}
