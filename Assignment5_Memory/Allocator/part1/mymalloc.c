...
void* mymalloc(size_t s){
	...
	heap_block = sbrk(total_size); // allocated total size
...
//	temp->in_use = 0;
	...
	programbreak = sbrk(0); // current heap break point
	...
	// if the current break point is equal to the break in memory 
		...
		sbrk(0 - BLOCK_SIZE - header->size); // get the previous location pointer
		pthread_mutex_unlock(&global_lock);
		return;
	}
